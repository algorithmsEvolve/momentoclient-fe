...
export default function BouquetPricing({ openViewer, sectionData }) {
  const packages =
    (sectionData?.packages || []).filter((item) => item?.enabled !== false);
  const fallbackPackages = pricingDefaults.sections.bouqet.packages;
