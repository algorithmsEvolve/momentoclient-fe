...
export default function BaraSimpleCountDown({ invitation }) {
          ...
          {justMarried ? (
            <div className={`just-married-wrapper ${isVisible ? "animate-zoom-in" : "opacity-0"}`} style={{ animationDelay: "300ms" }}>
              <div className="just-married">
                <img src="/themes/bara-simple/count-down/just-married-icon.svg" alt="just-married" />
              </div>
            </div>
          ) : (
            <>
              <div className={`icon ${isVisible ? "animate-zoom-in" : "opacity-0"}`} style={{ animationDelay: "200ms" }}>
                <img src="/themes/bara-simple/count-down/balloon-icon.svg" alt="day-icon" />
              </div>

              <div className={`title ${isVisible ? "animate-zoom-in" : "opacity-0"}`} style={{ animationDelay: "300ms" }}>
                <p>{isEN ? "The awaited day" : "Hari yang dinanti"}</p>
              </div>

              <div className={`count-down ${isVisible ? "animate-zoom-in" : "opacity-0"}`} style={{ animationDelay: "400ms" }}>
                {[
                  [isEN ? "Days" : "Hari", countdown.days],
                  [isEN ? "Hours" : "Jam", countdown.hours],
                  [isEN ? "Minutes" : "Menit", countdown.minutes],
                  [isEN ? "Seconds" : "Detik", countdown.seconds],
                ].map(([label, value]) => (
