 ...
 */
export default function YuugurePreloader({ progress = 0 }) {
  // Logo viewBox: Y dari 33 (atas) sampai 68 (bawah), tinggi 35 unit
  // progress=0   → wave di Y=68  (bawah logo, belum mengisi)
  // progress=100 → wave di Y=33  (atas logo, penuh terisi)
  const LOGO_TOP    = 28;   // sedikit di atas logo agar terisi penuh
  const LOGO_BOTTOM = 73;   // sedikit di bawah logo agar mulai dari kosong
  const waveY = LOGO_BOTTOM - ((progress / 100) * (LOGO_BOTTOM - LOGO_TOP));

  // Solid fill: selalu di bawah wave (Y = waveY → 110)
  const solidY     = waveY;
  const solidH     = 110 - solidY + 5;

  // Wave path: sinusoidal di waveY
  const amplitude = 3;
  const wavePath = [
    `M-20 ${waveY}`,
      ...
      <div className="preloader-content">
        <div className="logo-wave-wrapper">
          ...
          >
            ...
            <g clipPath="url(#logo-clip)" className="wave-fill-group">
