 ...
 * YamatoPreloader
 *
 * Displays Momento logo with wave fill effect.
 * Wave rises from bottom to top based on progress value (0-100).
 */
export default function YamatoPreloader({ progress = 0, invitation }) {
  ...
  const coverImage = isDesktop
    ? invitation?.settings?.coverImageDesktop
    : invitation?.settings?.coverImageMobile;
  const LOGO_TOP = 28;
  ...
  return (
    <div name="yamato-preloader" style={{ backgroundImage: coverImage ? `url('${coverImage}')` : undefined }}>
