...
export default function BaraRsvpForm({ invitation, guest }) {
    ...
    async function loadExistingRsvp() {
      ...
      if (!ignore && result.ok && result.data) {
        setForm({
          ...
          how: result.data.attendeeCount > 1 ? "berdua" : "sendiri",
    ...
    <div name="rsvp-section" ref={sectionRef}>
              ...
              <form onSubmit={handleSubmit} style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
                <div className={`desc ${isVisible ? "animate-zoom-in" : "opacity-0"}`} style={{ animationDelay: "350ms" }}>
                  <p>
                    ...
                    <span className="bold">{guest.name}</span>, merupakan suatu kehormatan apabila Bpk/ Ibu/ Sdr dapat hadir dalam acara kami. Mohon bantuannya agar kami dapat mempersiapkan segala sesuatunya secara maksimal dengan mengisi form berikut :
                  </p>
                </div>
                    ...
                    <div className="group-member-input">
                      ...
                      <div style={{ display: "flex", gap: "1rem" }}>
                        <label style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                          ...
                          />
                          <span style={{ fontFamily: "poppinsR", fontSize: "1.25rem", color: "#fff" }}>Hadir</span>
                        </label>
                        <label style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                          <input
                            type="radio"
                          ...
                          />
                          <span style={{ fontFamily: "poppinsR", fontSize: "1.25rem", color: "#fff" }}>Tidak Hadir</span>
                        </label>
                      </div>
                    </div>
                      ...
                      <div className="group-member-input">
                        ...
                        <div style={{ display: "flex", gap: "1rem" }}>
                          <label style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                            ...
                            />
                            <span style={{ fontFamily: "poppinsR", fontSize: "1.25rem", color: "#fff" }}>Sendiri</span>
                          </label>
                          <label style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                            <input
                              type="radio"
                            ...
                            />
                            <span style={{ fontFamily: "poppinsR", fontSize: "1.25rem", color: "#fff" }}>Berdua</span>
                          </label>
                        </div>
                      </div>
                    )}
      ...
      <div className="decorations">
        <div className={`top-left ${isVisible ? "animate-fade-in-right" : "opacity-0"}`} style={{ animationDelay: "1000ms" }}>
          <img src="/themes/bara/rsvp/decor-top-left.png" alt="decor-top-left" />
        </div>
        <div className={`top-right ${isVisible ? "animate-fade-in-left" : "opacity-0"}`} style={{ animationDelay: "1000ms" }}>
          <img src="/themes/bara/rsvp/decor-top-right.png" alt="decor-top-right" />
        </div>
      </div>
    </div>
  );
