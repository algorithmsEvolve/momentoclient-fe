...
export const buildSeserahanEstimationProducts = (section) => {
  ...
  const rawAddOnItems = Array.isArray(section.addOns?.items) ? section.addOns.items : [];
  const legacyBedcover = section.addOns?.bedcover;
  const hasBedcoverItem = rawAddOnItems.some(
    (item) => normalizeId(item?.id || item?.name) === "hias-bedcover"
  );
  const addOnItems =
    legacyBedcover && !hasBedcoverItem
      ? [...rawAddOnItems, legacyBedcover]
      : rawAddOnItems;
  ...
  const addOnProducts = addOnItems
    .filter((item) => item?.enabled !== false)
    .map((item, index) => {
      const price = parsePrice(item.price);
      const id = normalizeId(item.id || item.name || `addon-${index + 1}`);
      const isBedcover = id === "hias-bedcover";
      const isRingbox =
        typeof item.isRingbox === "boolean"
          ? item.isRingbox
          : !isBedcover;
      const baseName = item.name || (isRingbox ? "Ringbox" : "Add On");
      const name =
        isRingbox && !baseName.toLowerCase().includes("ringbox")
          ? `${baseName} Ringbox`
          : baseName;
      ...
      return {
        ...
        isRingbox,
        type: isBedcover ? "bedcover" : isRingbox ? "ringbox" : "seserahan-addon",
      };
    });

  const products = [...trays, ...addOnProducts];
